...
const MegaMenuSubNetwork = ({ items }) => {
  ...
  if (!networkItem) {
    return <div className="p-4">Menu not available</div>;
  }
  const networkItemGeneral = networkItem.children
    ? networkItem.children[0]
    : { children: [] };
  const networkItemOthers = networkItem.children ? networkItem.children[1] : { children: [] };
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="pt-4 pb-2 lg:p-7.5 lg:pb-5 grow">
          <div className="grid lg:grid-cols-2 gap-5">
            {networkItemOthers.children && networkItemOthers.children?.map((item, index) => {
              return (
