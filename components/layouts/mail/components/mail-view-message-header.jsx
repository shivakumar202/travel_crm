...
export function MailViewMessageHeader() {
  ...
  // Get the selected email data using current selected email
  const selectedEmailData = getSelectedEmailData(selectedEmailId);

  // Use selected email data or fallback to default
  const email = selectedEmailData || emailMessages[0];

  // Debug: Log when email changes
  useEffect(() => {
    console.log('Header updated with email:', email.sender, email.subject);
  }, [email.id]);
      ...
      <div className="px-4 mb-2">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          <Tooltip>
            ...
            <TooltipContent>
              <p>{email.sender} - Project Manager</p>
            </TooltipContent>
          </Tooltip>
        </div>
      </div>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex flex-col gap-2">
                    <span className="text-2sm text-muted-foreground">
                      ...
                      no-reply@{email.sender.toLowerCase()}.com
                    </span>
                    <span className="text-2sm">dr.creator@gmail.com</span>
                    <span className="text-2sm text-muted-foreground">
                      {email.sender} &lt;no-reply@{email.sender.toLowerCase()}
                      .com
                    </span>
                    <span className="text-2sm text-muted-foreground">
